...
<script setup>
	import {(ref, onMounted)} from 'vue' // This code runs ONCE when component is
	created const data = ref(null) console.log('Setup running') // Only logs once
	onMounted(() =>{' '}
	{
		console.log('Mounted') // Only logs once
	}
	) // If you need something to run on every reactive change, // use watch or
