...
/**
 * 勾选/取消勾选时，按"调拨单号 + 费用类型"维度联动当前页同组数据
 */
const handleCheckboxChange = (params: any) => {
	...
	// 获取 VXE Table 实例 需要用baseTable里抛出的tableRef,由于严格模式的原因，baseTableRef拿到的是组件实例而不是VXE Table实例
	const vxeTable = register.tableRef.value;
	if (!vxeTable) return;
	isSyncing = true;
...
const resetQuery = () => {
	register.state.queryForm = { pushState: 1 };
	getDataList();
};
...
const generateKdBill = async () => {
	try {
		// 获取勾选行，没有勾选则取当前页全部数据
		const selectedRows = register.tableRef.value?.getCheckboxRecords() || [];
		const selectedIds = register?.getSelectedIds() || [];
