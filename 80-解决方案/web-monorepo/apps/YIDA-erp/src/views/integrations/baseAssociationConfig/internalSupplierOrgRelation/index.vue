	...
	<div class="layout-padding">
		<div class="layout-padding-auto layout-padding-view">
			...
			<ImportDialog
				ref="importDialogRef"
				title="导入内部供应商与组织关系"
				:templateUrl="INTERNAL_SUPPLIER_ORG_RELATION_IMPORT_TEMPLATE_URL"
				:excelUploadApi="INTERNAL_SUPPLIER_ORG_RELATION_IMPORT_URL"
				:accept="'.xlsx,.xls'"
...
import {
	getInternalSupplierOrgRelationPage,
	INTERNAL_SUPPLIER_ORG_RELATION_IMPORT_TEMPLATE_URL,
	INTERNAL_SUPPLIER_ORG_RELATION_IMPORT_URL,
	getExportInternalSupplierOrgRelation,
	deleteInternalSupplierOrgRelation,
...
const state: BasicTableProps = reactive<BasicTableProps>({
	pageList: getInternalSupplierOrgRelationPage,
	columns,
	toolBarConfig: {
		handleRefresh: () => {
		...
		settingKey: 'internalSupplierOrgRelation',
		isShowSetting: true,
	},
...
const handleExport = async () => {
	try {
		...
		await getExportInternalSupplierOrgRelation({
			ids,
			...state.queryForm,
			ascs: state?.ascs,
			descs: state?.descs,
...
const handleDelete = async (_item: any, row: any) => {
	const res = await deleteInternalSupplierOrgRelation({
		id: row.id,
	});
	if (res.code === 0) {
