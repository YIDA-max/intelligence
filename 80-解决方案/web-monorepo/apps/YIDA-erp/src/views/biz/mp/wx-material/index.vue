				...
				<div class="layout-padding-auto layout-padding-view">
					<el-tabs v-model="materialType" @tab-click="handleClick">
						<el-tab-pane name="image" label="image">
							...
							<div
								v-if="state.dataList.length <= 0 && !state.loading"
						...
						<el-tab-pane name="voice" label="voice">
							<template #label
								><i class="el-icon-microphone"></i> 语音</template
							>
							<div class="add_but">
								<wx-file-upload
									@success="getDataList"
						...
						<el-tab-pane name="video" label="video">
							<template #label
								><i class="el-icon-video-play"></i> 视频</template
							>
							<div class="add_but">
							...
							<el-dialog title="新建视频" v-model="dialogVideoVisible">
								...
								<template #footer>
									<el-button @click="dialogVideoVisible = false"
										>取 消</el-button
									>
...
import {
	...
	getMaterialVideo,
	getPage,
} from '/@/api/mp/wx-material';

const QueryTree = defineAsyncComponent(
	() => import('/@/components/QueryTree/index.vue')
);
const NewsForm = defineAsyncComponent(
	() => import('./components/news-form.vue')
);
const WxFileUpload = defineAsyncComponent(
	() => import('/@/components/Wechat/fileUpload/index.vue')
);
const WxNews = defineAsyncComponent(
	() => import('/@/components/Wechat/wx-news/index.vue')
);
