			...
			<div class="title-box h-4">
				...
				<div class="action-box">
					<el-button
						...
						v-if="
							activeTab === 'estimate' &&
							auth('firstMileLogistics_showInfo_addFeeEstimated')
						"
					...
					<el-button
						...
						v-if="
							activeTab === 'actual' &&
							auth('firstMileLogistics_showInfo_addFeeActual')
						"
				...
				<el-row class="mb18">
					...
					<el-col :span="12"
						><el-form-item label="是否中转拆柜">
							<span>{{ currentRow?.transshipmentFlag ? '是' : '否' }}</span>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row class="mb18">
					<el-col :span="12">
						<el-form-item label="目的仓">
							<span>{{
								[...new Set(currentRow?.destWarehouseNameList)]?.join(', ') ||
								'--'
							}}</span>
						</el-form-item>
					</el-col>
				...
				<el-row class="mb18">
					<el-col :span="12">
						<el-form-item label="货代信息">
							<span>{{
								[
									currentRow?.domesticForwarderName &&
										`国内拖车：${currentRow?.domesticForwarderName}`,
									currentRow?.mainForwarderName &&
										`主运：${currentRow?.mainForwarderName}`,
									currentRow?.customsForwarderName &&
										`清关：${currentRow?.customsForwarderName}`,
									currentRow?.deliveryForwarderName &&
										`国外派送：${currentRow?.deliveryForwarderName}`,
								]
									.filter(Boolean)
									.join(' ｜ ') || '-'
							}}</span>
				...
				<div style="margin-top: 12px">
					...
					<div style="margin-top: 12px">
						...
						<template v-if="activeTab === 'delivery'">
							<MrTable
								:data="currentRow?.deliveryVOList || []"
								border
								height="400"
								style="width: 100%"
							...
							>
								...
								<MrTableColumn label="目的仓" width="200">
									<template #default="{ row }">
								...
								>
									<template #default="{ row }">
										{{ formatNumber(row.productNetWeight) }}
								...
								>
									<template #default="{ row }">
										{{ formatNumber(row.totalGrossWeight) }}
								...
								<MrTableColumn
									...
									width="180"
								/>
								<!-- <MrTableColumn label="备注" min-width="200" show-overflow-tooltip>
							<template #default="{ row }">
								{{ row.remark || '-' }}
							</template>
						</MrTableColumn> -->
						...
						<!-- 其他1 列表示例：可根据接口返回调整 columns/data -->
						<template
...
import {
	...
	transferFeeList,
	getTransferFeeList,
} from '/@/api/materialFlow/firstMileLogistics';
...
const id = ref<string | number | string[]>(normalizeQueryValue(route.query.id));
const transferNo = ref<string | number | string[]>(
	normalizeQueryValue(route.query.transferNo)
);
...
// 获取头程调拨详情
const fetchDetail = async (fetchId: string | number) => {
	if (!fetchId) return;
	try {
		...
		title.value =
			'头程调拨详情' +
			(currentRow.firstMileNo ? ` - ${currentRow.firstMileNo}` : '');
	} catch (e: any) {
...
onMounted(() => {
	const fetchId = (route.query.id ?? '') as string | number;
	fetchDetail(fetchId);
});
...
const getSummaries = ({ columns, data }: { columns: any[]; data: any[] }) => {
	...
	columns.forEach((column, index) => {
		...
		if (column.property === 'deliveryCartonQuantity') {
			const total = data.reduce(
				(sum, item) => sum + (Number(item.deliveryCartonQuantity) || 0),
				0
			);
			sums[index] = total;
		} else if (column.property === 'shipmentQuantity') {
			const total = data.reduce(
				(sum, item) => sum + (Number(item.shipmentQuantity) || 0),
				0
			);
			sums[index] = total;
		} else if (column.property === 'intoTotalCartonQuantity') {
			const total = data.reduce(
				(sum, item) => sum + (Number(item.intoTotalCartonQuantity) || 0),
				0
			);
			sums[index] = total;
		} else if (column.property === 'intoTotalQuantity') {
			const total = data.reduce(
				(sum, item) => sum + (Number(item.intoTotalQuantity) || 0),
				0
			);
			sums[index] = total;
		} else if (column.property === 'totalVolume') {
			const total = data
				.reduce((sum, item) => sum + (Number(item.totalVolume) || 0), 0)
				?.toFixed(2);
			sums[index] = total;
		} else if (column.property === 'totalNetWeight') {
			const total = data
				.reduce((sum, item) => sum + (Number(item.totalNetWeight) || 0), 0)
				?.toFixed(2);
			sums[index] = total;
		} else if (column.property === 'totalGrossWeight') {
			const total = data
				.reduce((sum, item) => sum + (Number(item.totalGrossWeight) || 0), 0)
				?.toFixed(2);
			sums[index] = total;
		} else {
			sums[index] = '';
...
// 打开费用录入弹窗（类型与 FeeDialog.open 一致）
