				...
				<div class="mb8" style="width: 100%">
					...
					>
						<el-tooltip
							class="item"
							effect="dark"
							:content="$t('queryTree.view')"
							placement="top"
						>
							<el-button
								circle
								icon="Grid"
								@click="tableViewRef = !tableViewRef"
							></el-button>
						</el-tooltip>
					</right-toolbar>
				</div>
				...
				>
					<template #default="scope">
						<dict-tag
							...
							:value="scope.row.jobExecuteStatus"
						></dict-tag>
					</template>
				</el-table-column>
						...
						<div class="grid grid-cols-4 gap-x-4 gap-y-6">
							...
							>
								...
								>
									<div
										aria-label="avatar"
										class="flex items-center mr-auto space-x-4"
									>
										<div class="flex flex-col flex-1 space-y-2">
											...
											<p
												class="text-base font-normal text-gray-500 truncate dark:text-gray-400"
											>
												{{ job.jobGroup }} /
												{{
													job_type.find(
														(item: any) => item.value === job.jobType
													)?.label
												}}
											</p>
										</div>
								...
								<div aria-label="navigation" class="py-2">
									<nav class="grid gap-1">
										<div
											class="flex items-center w-full px-4 py-1 space-x-3 leading-6 text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-[#303030] rounded-md"
										>
											<span class="font-medium"
												>{{ $t('job.jobStatus') }}:</span
											>
											<span>{{
												job_status.find(
													(item: any) => item.value === job.jobStatus
												)?.label
											}}</span>
										</div>
										<div
										...
										>
											<span class="font-medium"
												>{{ $t('job.jobExecuteStatus') }}:</span
											>
											<span>
												<el-icon v-if="job.jobExecuteStatus === '1'"
													><CircleCloseFilled
												/></el-icon>
												<el-icon v-if="job.jobExecuteStatus === '0'"
													><CircleCheckFilled
												/></el-icon>
											</span>
										</div>
								...
								<div
									aria-label="footer"
									class="pt-2 border-t dark:border-[#303030]"
								>
...
const exportExcel = () => {
	downBlobFile(
		'/job/sys-job/export',
		Object.assign(state.queryForm, { ids: selectedRows }),
		'job.xlsx'
	);
};
