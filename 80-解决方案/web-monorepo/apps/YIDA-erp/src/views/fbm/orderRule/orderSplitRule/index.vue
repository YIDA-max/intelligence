						...
						<el-switch
							:disabled="
								row.status === 1 || row.status === '1'
									? !auth('orderSplitRule-disable')
									: !auth('orderSplitRule-enable')
							"
							v-model="row.status"
							:active-value="1"
							:inactive-value="0"
							...
							<el-dropdown @command="(type:any) => openDialog(type, row)">
								...
								<template #dropdown>
									<el-dropdown-menu>
										<el-dropdown-item
											command="log"
											v-if="auth('orderSplitRule-info')"
											>日志</el-dropdown-item
										>
										<el-dropdown-item
											command="copy"
											v-if="auth('orderSplitRule-copy')"
											>复制</el-dropdown-item
										>
									</el-dropdown-menu>
								</template>
...
import { getRuleConditionSplitDetail } from '/@/api/fbm/orderRule/orderSplitRule/index';
...
const openDialog = async (type: string, row?: any) => {
	try {
		if (type === 'log') {
			...
			// 编辑规则
			const res = await getRuleConditionSplitDetail({
				id: row.id,
				type: 'split',
			});
			if (res.code === 0) {
			...
			ElMessageBox.confirm(
				...
				.then(async () => {
					let res;
					if (row.status === 1) {
						res = await postRuleInfoOpen({ ids: [row.id], type: 'split' });
					} else {
						res = await postRuleInfoClose({ ids: [row.id], type: 'split' });
					}
					if (res.code === 0) {
