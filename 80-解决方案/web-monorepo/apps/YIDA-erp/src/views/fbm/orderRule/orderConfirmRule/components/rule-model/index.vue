 ...
 * @LastEditTime: 2026-04-18 15:42:55
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderConfirmRule\components\rule-model\index.vue
 * @Description: 
 * 
...
const onSubmit = async () => {
	try {
			...
			ruleConditionAccept: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionAccept.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionAccept.ruleId
						: undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionAccept: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionAccept.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionAccept.ruleId
						: undefined, // 规则id
			},
			type: 'accept', // 确定是哪个规则单
		...
		const hasAnyCondition = Object.entries(params.ruleConditionAccept).some(
			([key, val]) =>
	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" :modelTitle="t('接单规则')" />
				</el-col>
			</el-row>
