				...
				>
					<template #left-tool>
						...
						<el-button
							...
							"
							v-auth="'pushExceptionRule-enable'"
						>
							{{ $t('批量启用') }}</el-button
						>
						<el-button
							...
							"
							v-auth="'pushExceptionRule-disable'"
						>
							{{ $t('批量禁用') }}</el-button
						>
					...
					<template #default="{ row }">
						<div class="flex items-center">
							<el-link
								...
								v-if="auth('pushExceptionRule-edit')"
							>
								{{ $t('编辑') }}
							</el-link>
							<el-dropdown @command="(type:any) => openDialog(type, row)">
								...
								<template #dropdown>
									<el-dropdown-menu>
										<el-dropdown-item
											command="log"
											v-if="auth('pushExceptionRule-info')"
											>日志</el-dropdown-item
										>
										<el-dropdown-item
											command="copy"
											v-if="auth('pushExceptionRule-copy')"
											>复制</el-dropdown-item
										>
									</el-dropdown-menu>
								</template>
...
import { getRulePushExceptionDetail } from '/@/api/fbm/orderRule/pushExceptionRule/index';
...
const openDialog = async (type: string, row?: any) => {
	try {
		if (type === 'log') {
			...
			// 编辑规则
			const res = await getRulePushExceptionDetail({
				id: row.id,
			});
			if (res.code === 0) {
			...
			ElMessageBox.confirm(
				...
				.then(async () => {
					let res;
					if (row.status === 1) {
						res = await postRuleInfoOpen({ ids: [row.id], type: 'wms' });
					} else {
						res = await postRuleInfoClose({ ids: [row.id], type: 'wms' });
					}
					if (res.code === 0) {
			...
			ElMessageBox.confirm(t('是否确认启用选中的规则？'), {
				...
				.then(async () => {
					const res = await postRuleInfoOpen({ ids: ids, type: 'wms' });
					if (res.code === 0) {
						useMessage().success(t('操作成功'));
						getDataList();
					} else {
			...
			ElMessageBox.confirm(t('是否确认禁用选中的规则？'), {
				...
				.then(async () => {
					const res = await postRuleInfoClose({ ids: ids, type: 'wms' });
					if (res.code === 0) {
						useMessage().success(t('操作成功'));
						getDataList();
					} else {
