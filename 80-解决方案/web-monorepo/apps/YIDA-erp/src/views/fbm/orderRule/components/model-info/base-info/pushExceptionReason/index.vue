...
const rules = ref({
	exceptionKeywords: [
		{ required: true, message: '请选择推单异常原因关键词', trigger: 'blur' },
	],
});
const init = () => {
	// 重置数据
...
const handleBlur = async () => {
	...
	await nextTick();
	// 如果有逗号分隔的项，进行拆分并去重
	const valuesArr = Array.from(formData.value.exceptionKeywords) as string[];
	if (valuesArr && valuesArr.length > 0) {
		...
		// 去重
		formData.value.exceptionKeywords = Array.from(new Set(newValues));
	} else {
		formData.value.exceptionKeywords = [];
	}
};
...
const setFormData = (data: Record<string, any>) => {
	// 设置数据
	return isInToSetFormData(data, 'exceptionKeywords', formData);
};
// 完善的数据设置函数
		...
		<el-form :model="formData" ref="formRef" label-width="150px" :rules="rules">
			<el-row class="base-info" :gutter="20">
				<el-col :span="24">
					<el-form-item
						:label="t('推单异常原因关键词')"
						prop="exceptionKeywords"
					>
						<el-row style="width: 100%">
							<el-col :span="24">
								支持批量复制不同分项按逗号分隔,按回车(Enter)键或点击鼠标完成输入
								(已填写{{ formData.exceptionKeywords.length }} 项)
							</el-col>
						</el-row>
						<el-row style="width: 100%">
							<el-col :span="4">
							...
							<el-col :span="20">
								<el-input-tag
									v-model="formData.exceptionKeywords"
									trigger="Enter"
									@blur="handleBlur"
								/>
							</el-col>
