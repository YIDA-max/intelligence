			...
			<el-row>
				...
				>
					<template #left-tool>
						...
						>
						<el-button
							v-auth="'orderStandardRule-enable'"
							@click="
								() => {
						...
						>
						<el-button
							v-auth="'orderStandardRule-disable'"
							@click="
								() => {
						...
						<el-switch
							:disabled="
								row.status === 1 || row.status === '1'
									? !auth('orderStandardRule-disable')
									: !auth('orderStandardRule-enable')
							"
							v-model="row.status"
							:active-value="1"
							:inactive-value="0"
					...
					<template #default="{ row }">
						<div class="flex items-center">
							<el-link
								...
								v-if="auth('orderStandardRule-edit')"
							>
								{{ $t('编辑') }}
							</el-link>
							<el-dropdown @command="(type:any) => openDialog(type, row)">
								...
								<template #dropdown>
									<el-dropdown-menu>
										<el-dropdown-item
											command="log"
											v-if="auth('orderStandardRule-info')"
											>日志</el-dropdown-item
										>
										<el-dropdown-item
											command="copy"
											v-if="auth('orderStandardRule-copy')"
											>复制</el-dropdown-item
										>
									</el-dropdown-menu>
								</template>
...
import { getRuleConditionShippedDetail } from '/@/api/fbm/orderRule/orderStandardRule/index';
...
const openDialog = async (type: string, row?: any) => {
	try {
		if (type === 'log') {
			...
			// 编辑规则
			const res = await getRuleConditionShippedDetail({
				id: row.id,
				type: 'shipped',
			});
			if (res.code === 0) {
			...
			ElMessageBox.confirm(
				...
				.then(async () => {
					let res;
					if (row.status === 1) {
						res = await postRuleInfoOpen({ ids: [row.id], type: 'shipped' });
					} else {
						res = await postRuleInfoClose({ ids: [row.id], type: 'shipped' });
					}
					if (res.code === 0) {
