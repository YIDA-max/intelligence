...
// 表单校验
const rules = ref({
	recipientCity: [{ required: true, message: '请选择收货城市', trigger: 'blur' }],
});
const init = () => {
	// 重置数据
	formData.value = {
...
const handleBlur = async () => {
	...
	await nextTick();
	// 如果有逗号分隔的项，进行拆分并去重
	const valuesArr = Array.from(formData.value.recipientCity) as string[];
	if (valuesArr && valuesArr.length > 0) {
		...
		// 去重
		formData.value.recipientCity = Array.from(new Set(newValues));
	} else {
		formData.value.recipientCity = [];
	}
};
...
const validateForm = async () => {
	...
	return isInToValidateForm(formData.value.isBelong, 'recipientCity', formData.value.recipientCity);
};
// 设置回显值
const setFormData = (data: Record<string, any>) => {
	// 设置数据
	return isInToSetFormData(data, 'recipientCity', formData);
};
// 完善的数据设置函数
	...
	<div class="box">
		<el-form :model="formData" ref="formRef" label-width="150px" :rules="rules">
			<el-row class="base-info" :gutter="20">
				<el-col :span="24">
					<el-form-item :label="t('收货城市')" prop="recipientCity">
						<el-row style="width: 100%">
							<el-col :span="24">
								支持批量复制不同分项按逗号分隔,按回车(Enter)键或点击鼠标完成输入 (已填写{{ formData.recipientCity.length }} 项)
							</el-col>
						</el-row>
						<el-row style="width: 100%">
							<el-col :span="4">
							...
							<el-col :span="20">
								<el-input-tag v-model="formData.recipientCity" trigger="Enter" @blur="handleBlur" />
							</el-col>
						</el-row>
					</el-form-item>
				</el-col>
