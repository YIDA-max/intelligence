	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" modelTitle="仓库规则" />
				</el-col>
			</el-row>
...
} from '/@/api/fbm/orderRule/orderWarehouseRule';
...
const onSubmit = async () => {
	try {
		...
		// debugger;
		// 设定动作数据
		const actionResult = await setAction.value?.validateForm();
		if (!actionResult?.valid) {
			useMessage().error(actionResult?.message || '设定动作验证失败');
			return;
		}
		...
		const params = {
			...
			ruleConditionWarehouseDTO: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionWarehouseVO.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionWarehouseVO.ruleId
						: undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionWarehouseDTO: {
				...actionData, // 设定动作数据
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionWarehouseVO.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionWarehouseVO.ruleId
						: undefined, // 规则id
			},
		...
		const hasAnyCondition = Object.entries(
			params.ruleConditionWarehouseDTO
		).some(
			([key, val]) =>
