...
const validateForm = async () => {
	...
	if (Number(formData.value.minPrice) > Number(formData.value.maxPrice) && formData.value.maxPrice && formData.value.minPrice) {
		...
		const getFormData = {
			sellingPrice: `sellingPrice >= ${formData.value.minPrice} && sellingPrice <= ${formData.value.maxPrice}`,
		};
		return getFormData;
	} else if (formData.value.minPrice && !formData.value.maxPrice) {
		const getFormData = {
			sellingPrice: `sellingPrice >= ${formData.value.minPrice}`,
		};
		return getFormData;
	} else if (!formData.value.minPrice && formData.value.maxPrice) {
		const getFormData = {
			sellingPrice: `sellingPrice <= ${formData.value.maxPrice}`,
		};
		return getFormData;
	}
};
...
const setFormData = (data: Record<string, any>) => {
	if (!data || !data.sellingPrice) {
		return '错误：无效的数据格式，缺少 sellingPrice 字段';
	}

	const priceStr = data.sellingPrice;

	// 使用正则表达式解析价格范围
	const regex = /sellingPrice\s*([><]=?)\s*([\d.]+)(?:\s*&&\s*sellingPrice\s*([><]=?)\s*([\d.]+))?/;
