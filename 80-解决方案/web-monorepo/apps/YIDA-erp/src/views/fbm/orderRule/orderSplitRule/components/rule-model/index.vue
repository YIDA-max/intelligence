 ...
 * @LastEditTime: 2026-05-28 15:07:58
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderSplitRule\components\rule-model\index.vue
 * @Description: 
 * 
...
import {
	postRuleConditionSplitUpdate,
	postRuleConditionSplitSave,
} from '/@/api/fbm/orderRule/orderSplitRule';
...
const onSubmit = async () => {
	try {
			...
			ruleConditionSplit: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionSplit.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionSplit.ruleId
						: undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionSplit: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionSplit.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionSplit.ruleId
						: undefined, // 规则id
			},
			type: 'split', // 确定是哪个规则单
		...
		const hasAnyCondition = Object.entries(params.ruleConditionSplit).some(
			([key, val]) =>
	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" :model-title="t('拆单规则')" />
				</el-col>
			</el-row>
