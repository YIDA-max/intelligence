			...
			<el-row>
				...
				>
					<template #left-tool>
						<el-button
							type="primary"
							v-auth="'orderReceiptInformation-add'"
							@click="
								() => {
									openDialog('add');
								}
						...
						>
						<el-button
							v-auth="'orderReceiptInformation-enable'"
							@click="
								() => {
						...
						>
						<el-button
							v-auth="'orderReceiptInformation-disable'"
							@click="
								() => {
						...
						<el-switch
							:disabled="
								row.status === 1 || row.status === '1'
									? !auth('orderReceiptInformation-disable')
									: !auth('orderReceiptInformation-enable')
							"
							v-model="row.status"
							:active-value="1"
							:inactive-value="0"
					...
					<template #default="{ row }">
						<div class="flex items-center">
							<el-link
								...
								v-if="auth('orderReceiptInformation-edit')"
							>
								{{ $t('编辑') }}
							</el-link>
							<el-dropdown @command="(type:any) => openDialog(type, row)">
								...
								<template #dropdown>
									<el-dropdown-menu>
										<el-dropdown-item
											command="log"
											v-if="auth('orderReceiptInformation-info')"
											>日志</el-dropdown-item
										>
										<el-dropdown-item
											command="copy"
											v-if="auth('orderReceiptInformation-copy')"
											>复制</el-dropdown-item
										>
									</el-dropdown-menu>
								</template>
...
import { getRuleConditionRecipientDetail } from '/@/api/fbm/orderRule/orderReceiptInformation/index';
...
const { dictMap } = useMultipleDicts(
	[
		...
		}, // 三方仓仓库列表
		{
			...
			params: { current: 1, size: 1000000000000 },
		}, // 物流渠道列表
	],
	{
		isAuto: true,
...
const openDialog = async (type: string, row?: any) => {
	try {
		if (type === 'log') {
			...
			// 编辑规则
			const res = await getRuleConditionRecipientDetail({
				id: row.id,
				type: 'recipient',
			});
			if (res.code === 0) {
			...
			ElMessageBox.confirm(
				...
				.then(async () => {
					let res;
					if (row.status === 1) {
						res = await postRuleInfoOpen({ ids: [row.id], type: 'recipient' });
					} else {
						res = await postRuleInfoClose({ ids: [row.id], type: 'recipient' });
					}
					if (res.code === 0) {
