 ...
 * @LastEditTime: 2026-05-28 15:07:06
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderAuditRule\components\rule-model\index.vue
 * @Description: 
 * 
...
} from '/@/api/fbm/orderRule/orderAuditRule/index';
...
// 运费售价比上限
const ShippingCostToPrice = defineAsyncComponent(
	() =>
		import(
			'/@/views/fbm/orderRule/components/model-info/warehouse-shipping/shippingCostToPrice/index.vue'
		)
);
...
// 运费售价比上限ref
const shippingCostToPriceRef = ref<any>(null);
// 仓库物流ref
...
const optionsGroups = ref([
	...
	},
	{
		...
		options: [
			...
			{ label: '运费售价比上限', value: 'shippingCostToPrice' },
...
const init = () => {
	...
	initFn(props, {
		...
		shippingCostToPriceRef, // 运费售价比上限ref
...
const onSubmit = async () => {
	try {
		...
		//   获取运费售价比上限数据
		const shippingCostToPriceData = currentCheckList.value.includes(
			'shippingCostToPrice'
		)
			? (await shippingCostToPriceRef.value?.validateForm()) || {}
			: {
		...
		const params = {
			...
			ruleConditionReview: {
				...
				...shippingCostToPriceData, // 运费售价比上限数据
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionReview.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionReview.ruleId
						: undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionReview: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionReview.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionReview.ruleId
						: undefined, // 规则id
			},
			type: 'review', // 确定是哪个规则单
		...
		const hasAnyCondition = Object.entries(params.ruleConditionReview).some(
			([key, val]) =>
	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" modelTitle="审核规则" />
				</el-col>
			</el-row>
			<el-row class="rule-content" :gutter="20">
				...
				<el-col :span="19" class="rule-info">
					...
					<el-row v-show="currentCheckList?.includes('shippingCostToPrice')">
						<el-col :span="24">
							<!-- 运费售价比上限 -->
							<ShippingCostToPrice ref="shippingCostToPriceRef" />
						</el-col>
					</el-row>
					<el-row
