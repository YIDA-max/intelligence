 ...
 * @LastEditTime: 2025-12-12 18:23:29
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\components\model-info\warehouse-shipping\shipTocalFee\index.vue
 * @Description: 
 * 
...
const validateForm = async () => {
	...
	if (
		...
		const getFormData = {
			calFee: `calFee >= ${formData.value.minPrice} && calFee <= ${formData.value.maxPrice}`,
		};
		return getFormData;
	} else if (formData.value.minPrice && !formData.value.maxPrice) {
		const getFormData = {
			calFee: `calFee >= ${formData.value.minPrice}`,
		};
		return getFormData;
	} else if (!formData.value.minPrice && formData.value.maxPrice) {
		const getFormData = {
			calFee: `calFee <= ${formData.value.maxPrice}`,
		};
		return getFormData;
	}
};
...
const setFormData = (data: Record<string, any>) => {
	if (!data || !data.calFee) {
		return '错误：无效的数据格式，缺少 calFee 字段';
	}

	const priceStr = data.calFee;

	// 使用正则表达式解析价格范围
	const regex =
		/calFee\s*([><]=?)\s*([\d.]+)(?:\s*&&\s*calFee\s*([><]=?)\s*([\d.]+))?/;
