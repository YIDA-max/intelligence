				...
				>
					<template #left-tool>
						...
						<el-button
							...
							"
							v-auth="'orderWarehouseRule-enable'"
						>
							{{ $t('批量启用') }}</el-button
						>
						<el-button
							...
							"
							v-auth="'orderWarehouseRule-disable'"
						>
							{{ $t('批量禁用') }}</el-button
						>
						...
						<el-switch
							:disabled="
								row.status === 1 || row.status === '1'
									? !auth('orderWarehouseRule-disable')
									: !auth('orderWarehouseRule-enable')
							"
							v-model="row.status"
							:active-value="1"
							:inactive-value="0"
					...
					<template #default="{ row }">
						<div class="flex items-center">
							<el-link
								...
								v-if="auth('orderWarehouseRule-edit')"
								>{{ $t('编辑') }}
							</el-link>
							<el-dropdown @command="(type:any) => openDialog(type, row)">
								...
								<template #dropdown>
									<el-dropdown-menu>
										<el-dropdown-item
											command="log"
											v-if="auth('orderWarehouseRule-info')"
											>日志</el-dropdown-item
										>
										<el-dropdown-item
											command="copy"
											v-if="auth('orderWarehouseRule-copy')"
											>复制</el-dropdown-item
										>
									</el-dropdown-menu>
								</template>
...
import {
	getRuleInfoList,
	reorderRuleInfoList,
	getRuleConditionShippedDetail,
} from '/@/api/fbm/orderRule/orderWarehouseRule/index';
...
const openDialog = async (type: string, row?: any) => {
	try {
		if (type === 'log') {
			...
			// 编辑规则
			const res = await getRuleConditionShippedDetail(row.id);
			if (res.code === 0) {
			...
			ElMessageBox.confirm(
				...
				.then(async () => {
					let res;
					if (row.status === 1) {
						res = await postRuleInfoOpen({ ids: [row.id], type: 'warehouse' });
					} else {
						res = await postRuleInfoClose({ ids: [row.id], type: 'warehouse' });
					}
					if (res.code === 0) {
