				...
				<el-row :gutter="20">
					...
					<el-col :span="12" class="mb20">
						<el-form-item :label="$t('sysuser.dept')" prop="deptIds">
							<Deptids :deptData="deptData" ref="deptidsRef" />
						</el-form-item>
					</el-col>
...
const Deptids = defineAsyncComponent(
	() => import('/@/views/admin/system/user/components/deptids/index.vue')
);
const { t } = useI18n();
...
const dataRules = ref({
	...
	username: [
		...
		},
		{
			validator: (rule: any, value: any, callback: any) => {
				validateUsername(rule, value, callback, dataForm.value.userId !== '');
			},
			trigger: 'blur',
		},
	],
	...
	deptIds: [{ required: true, message: '部门不能为空', trigger: 'blur' }],
	...
	phone: [
		...
		{
			validator: (rule: any, value: any, callback: any) => {
				validatePhone(rule, value, callback, dataForm.value.userId !== '');
			},
			trigger: 'blur',
		},
	],
...
// 监听部门变化
watch(
	() => deptidsRef.value?.deptIds,
	(newDeptIds) => {
		dataForm.value.deptIds = newDeptIds;
	}
);
...
const openDialog = async (id: string) => {
	visible.value = true;
	// 等待子组件加载完成
	const isReady = await useWaitForRefsReady([deptidsRef]);
	if (!isReady) {
		useMessage().error('组件未加载完成，请稍后再试');
	}
	// 清空数据
	dataForm.value.userId = '';
	deptidsRef.value.setData([]);
	// 重置表单数据
	nextTick(() => {
...
const onSubmit = async () => {
	...
	try {
		const { userId, phone, password } = dataForm.value;

		if (userId) {
			// 清除占位符，避免提交错误的数据
			if (phone?.includes('*')) dataForm.value.phone = undefined;
			if (password?.includes('******')) dataForm.value.password = undefined;
			// 设置部门数据
			dataForm.value.deptIds = deptidsRef.value?.getData() || [];
			loading.value = true;
			const res = await putObj(dataForm.value);
			if (res.code === 0) {
			...
			loading.value = true;
			const res = await addObj(dataForm.value);
			if (res.code === 0) {
...
const getUserData = async (id: string) => {
	try {
		...
		if (data.roleList) {
			dataForm.value.role = data.roleList.map(
				(item: { roleId: any }) => item.roleId
			);
		}
		if (data.postList) {
			dataForm.value.post = data.postList.map(
				(item: { postId: any }) => item.postId
			);
		}
		// 处理部门数据的回显
		if (data.deptIds) {
			deptidsRef.value?.setData(data.deptIds);
		}
	} catch (err: any) {
...
const getPostData = () => {
	postList().then((res) => {
		...
		if (!dataForm.value.userId) {
			dataForm.value.post = [res.data[0].postId];
		}
	});
};
// 角色数据
const getRoleData = () => {
	roleList().then((res) => {
		...
		// 默认选择第一个
		if (!dataForm.value.userId) {
			dataForm.value.role = [res.data[0].roleId];
		}
	});
};
