...
export const rule = {
	...
	/**
	 * 校验用户输入的长度在 1-100 字符之间，允许空格
	 * @param rule 校验规则
	 * @param value 输入的值
	 * @param callback 校验回调
	 */
	validateLength(rule: any, value: any, callback: any) {
		if (value.length < 1 || value.length > 100) {
			callback(new Error('输入内容长度必须在 1 到 100 字符之间'));
		} else {
			callback();
		}
	},
	/**
	 * 校验 支持数字、字母、下划线（_）、减号（-）、英文点（.）、井号（#）
	 * 名称校验
	 */
	validatorCode(rule: any, value: any, callback: any) {
		const acount = /^[a-zA-Z0-9_\-.#]+$/;
		if (value && !acount.test(value)) {
			callback(new Error('支持数字 字母 _ - . #'));
		} else {
			callback();
		}
	},
	 ...
	 * 校验邮政编码
	 */
	validatePostalCode(rule: any, value: any, callback: any) {
		// 邮政编码正则（6位数字）
		const isPostalCode = /^\d{6}$/;

		if (value.indexOf('****') >= 0) {
			return callback(); // 忽略 "****" 情况
		}
