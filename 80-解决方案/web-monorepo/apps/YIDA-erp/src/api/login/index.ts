...
export const login = (data: any) => {
	...
	// 密码加密
	const encPassword = other.encryption(
		data.password,
...
export const loginByMobile = (mobile: any, code: any) => {
	...
	return request({
		...
		method: 'post',
		params: {
			mobile: `${SocialLoginEnum.SMS}@${mobile}`,
			code: code,
			grant_type,
			scope,
		},
...
export const loginBySocial = (state: SocialLoginEnum, code: string) => {
	...
	const scope = 'server';
	const basicAuth =
		'Basic ' + window.btoa(import.meta.env.VITE_OAUTH2_SOCIAL_CLIENT);
	Session.set('basicAuth', basicAuth);
 ...
 * 获取用户部门
 */
export const getUserWithDeptByCondition = (params: any) => {
	return request({
		url: '/admin/user/getUserWithDeptByCondition',
		method: 'get',
		params,
	});
};
