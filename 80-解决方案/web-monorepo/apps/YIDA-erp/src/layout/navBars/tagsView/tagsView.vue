...
// 当前的 tagsView 项点击时
const onTagsClick = (tag: any) => {
	const resolved = router.resolve({
		path: tag.path,
		query: tag.query,
	});

	// 路由完全不存在，或者路由的 name,path 发生了变化，都视为失效
	const isRouteValid = router.getRoutes().some((route) => {
		return route.path === tag.path;
	});

	const isInvalid =
		resolved.matched.length === 0 ||
		(tag.name && resolved.name !== tag.name) ||
		!isRouteValid;

	if (isInvalid) {
		ElMessage({
			type: 'warning',
			message: '检测到路由发生变化，该页面已失效',
		});

		// 删除失效的 tag
		closeCurrentTagsView(
			getThemeConfig.value.isShareTagsView ? tag.path : tag.url
		);
