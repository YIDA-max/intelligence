<template>
	<div class="material">
										...
										<template #dropdown>
											<el-dropdown-menu>
												...
												>
													<div>
														<el-dropdown-item>
															{{ $t('material.editGroup') }}</el-dropdown-item
														>
													</div>
												</popover-input>
		...
		<div class="flex flex-col material__center">
			<div class="flex operate-btn">
				...
				<el-input
					class="mr-16 ml-80"
					:placeholder="$t('file.inputfileNameTip')"
					v-model="fileParams.original"
					@keyup.enter="refresh"
				>
					<template #append>
	...
	<el-dialog
		:title="$t('material.uploadFileTip')"
	...
	>
		<upload-file
			...
			:data="{ groupId: cateId, type: typeValue }"
...
const Popup = defineAsyncComponent(
	() => import('/@/components/Popup/index.vue')
);
...
const handleDownFile = (item: any) => {
	other.downBlobFile(
		`/admin/sys-file/oss/file?fileName=${item.fileName}`,
		{},
		item.original
	);
};
...
const getFileUri = (item: any) => {
	return `${(proxy as any)?.baseURL}/admin/sys-file/oss/file?fileName=${
		item.fileName
	}`;
};
