<template>
	<div class="z-[1000]" v-if="getThemeConfig.isChat">
		<button
			...
			class="fixed inline-flex items-center justify-center w-16 h-16 p-0 m-0 mb-16 mr-4 text-sm font-medium leading-5 normal-case border border-gray-200 rounded-full cursor-pointer right-4 bottom-4 bg-none dark:border-gray-600 bg-primary dark:bg-gray-700 hover:bg-gray-700 dark:hover:bg-gray-600 disabled:pointer-events-none disabled:opacity-50"
			type="button"
		...
		<div v-show="chatContainer" class="fixed bottom-24 right-4 w-[600px] mb-12">
			<div class="w-full bg-white rounded-lg shadow-lg dark:bg-gray-800">
				<div
					class="flex items-center justify-between p-4 text-white border-b border-gray-200 rounded-t-lg dark:border-gray-600 bg-primary dark:bg-gray-700"
				>
					<p class="text-lg font-semibold">{{ t('chat.title') }}</p>
					<div class="flex items-center space-x-2">
						<button
							@click="clearChat"
							class="flex items-center justify-center w-6 h-6 text-gray-300 focus:outline-none hover:text-gray-400 dark:text-gray-400 dark:hover:text-gray-300"
					...
					<div v-for="(message, index) in messageList" :key="index" class="space-y-2">
						...
						<div v-if="message.botMessage || message.reasoningChain || message.isTyping" class="flex flex-col space-y-2">
							...
							<div v-if="message.reasoningChain" class="overflow-hidden bg-gray-100 rounded-lg dark:bg-gray-700">
								<div
									class="flex items-center justify-between p-2 transition-colors cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-600"
								...
								<div
									...
									class="p-3 text-sm text-gray-600 border-t border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300"
								>
				...
				<div class="flex items-center p-4 space-x-2 border-t border-gray-200 dark:border-gray-600">
					<button
						...
						class="relative flex items-center justify-center w-10 h-10 text-gray-400 transition-colors bg-gray-100 rounded-md group dark:bg-gray-700 dark:text-gray-400"
						:class="{
					...
					>
						...
						<div
							class="absolute invisible px-3 py-2 mb-2 text-sm text-white transition-all duration-200 -translate-x-1/2 bg-gray-800 rounded-lg opacity-0 bottom-full left-1/2 whitespace-nowrap group-hover:opacity-100 group-hover:visible"
						>
							{{ isWebEnabled ? t('chat.webSearchEnabled') : t('chat.webSearchDisabled') }}
							<div class="absolute -mt-1 -translate-x-1/2 border-4 border-transparent top-full left-1/2 border-t-gray-800"></div>
						</div>
					</button>
					<input
						v-model="userInput"
						...
						:readonly="readonly"
						class="w-full px-3 py-2 text-gray-900 border border-gray-200 bg-gray-50 rounded-l-md dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary"
					/>
					<button
						@click="sendMessage"
						class="px-4 py-2 text-white transition duration-300 rounded-r-md disabled:opacity-50 bg-primary hover:bg-primary-focus dark:bg-primary-light-3 dark:hover:bg-primary-light-5"
						:disabled="readonly"
					>
						{{ t('chat.send') }}
					</button>
