 ...
 * @LastEditTime: 2026-04-29 11:04:41
 * @FilePath: \qianyi-ui\src\components\FilePreviewDialog\usePreviewDialog\index.ts
 * @Description:
 *
 * Copyright (c) 2025 by ${git_name_email}, All Rights Reserved.
 */
// src/hooks/usePreviewDialog/index.ts
...
export function usePreviewDialog() {
	...
	if (!inst) throw new Error('usePreviewDialog() 必须在组件 setup 中调用');
	...
	const ensureInstance = (onClosed: () => void) => {
		...
		// 创建虚拟节点，并绑定关闭回调
		vnode = createVNode(FilePreviewDialog, {
	 ...
	 */
	const open = async (previewSrc: string) => {
		// 确保实例已创建
