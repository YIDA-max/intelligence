<template>
	...
	>
		...
		<el-form :model="formData" ref="formRef" :inline="false" label-position="left">
			<el-form-item :label="selectLabel" :prop="selectProp">
				<el-select v-model="formData[selectProp]" :placeholder="selectPlaceholder" style="width: 100%">
					<el-option v-for="item in options" :key="item[optionProps.value]" :label="item[optionProps.label]" :value="item[optionProps.value]" />
				</el-select>
			</el-form-item>
		</el-form>
	</confirm-dialog>
...
const initFormData = () => {
	...
	// 设置新的初始值
	formData[props.selectProp] = props.modelValue;
};

// 监听表单值变化
watch(
	() => formData[props.selectProp],
	(newVal) => {
