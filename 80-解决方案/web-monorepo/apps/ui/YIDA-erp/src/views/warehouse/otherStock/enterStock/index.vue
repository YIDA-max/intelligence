...
import {
	getOtherInboundPage,
	getOtherInboundById,
	getOtherInboundDetails,
	postOtherInboundLogisticsAudit,
	postOtherInboundFinancialAudit,
	postOtherInboundDiscard,
	getOtherInboundExport,
...
const openDialog = async (type: string, row?: any) => {
	try {
		...
		if (type === 'add') {
			...
			// 如果状态不是审核驳回，则不可编辑
			if (row.status !== 4) {
				useMessage().error(t('只有审核驳回状态的需求才能编辑'));
				return;
			}
			const res = await getOtherInboundById({ id: row.id });
			if (res.code === 0) {
				formDialogRef.value.show({
					title: t('编辑其他入库单'),
					visibleType: 'edit',
			...
			ElMessageBox.confirm(
				...
				try {
					const res = await postOtherInboundDiscard({ ids });
					if (res.code === 0) {
						// 判断是否有失败的记录
...
const handleConfirm = async (type: string, e?: any) => {
	...
	try {
		if (type === 'material') {
			// 物流审核
			const res = await postOtherInboundLogisticsAudit({
				ids,
			...
			// 财务审核
			const res = await postOtherInboundFinancialAudit({
				ids,
				passFlag: 1, //  是否审核通过 0否，1是
				remark: e.remark,
			});
...
const clickRowHandle = async (row: any) => {
	...
	if (!row.skuList?.length) {
		const data = await getOtherInboundDetails({
			inboundNo: row.inboundNo,
		});
		const list = getTableData();

		list?.map((item) => {
			if (item.inboundNo === row.inboundNo) {
