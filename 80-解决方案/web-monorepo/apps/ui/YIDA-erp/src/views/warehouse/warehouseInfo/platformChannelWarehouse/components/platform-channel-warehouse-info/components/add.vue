				...
				<el-row class="mb18">
					<el-col :span="12">
						<el-form-item label="仓库类型" prop="warehouseType">
							...
							>
								<el-option
									v-for="item in [{ value: '2', label: '平台仓' }]"
									:key="item.value"
									:label="item.label"
									:value="item.value"
								/>
					...
					<el-col :span="12">
						<el-form-item label="就近港口" prop="warehousePortCode">
							<SelectInputApi
								v-model="formData.warehousePortCode"
...
const formData = ref<{
	...
	companyName: string;
	warehousePortCode: string | number | null;
...
const rules = ref({
	...
	warehousePortCode: [
		{ required: true, message: '就近港口不能为空', trigger: 'change' },
	],
	platformChannelCode: [
...
const handleOpen = async () => {
	...
	// 设置默认仓库类型为海外仓
	formData.value.warehouseType = '2';
};
