<template>
	<div class="layout-padding-auto layout-padding-view">
		...
		<el-row>
			<ToolBar @handleRefresh="resetQuery" @handleExport="exportPartFn">
				<template #left-tool>
					<el-button
						class="ml10"
						type="primary"
		...
		>
			...
			>
				<template #default="{ row }">
					<span>{{ row.platformChannelCodeText }}</span>
				</template>
			</el-table-column>
			<el-table-column
				prop="warehouseName"
				...
				<template #default="{ row }">
					<div class="flex items-center">
						<el-button
							...
							v-auth="'platformChannelWarehouse_edit'"
							>{{ $t('编辑') }}
						</el-button>
						<el-button
							text
...
import {
	getWareHouseList,
	getPlatformShow,
	getExportWarehouseExcel,
	postDisableWareHouse,
	postEnableWareHouse,
...
const openDialog = async (type: string, row?: any) => {
	if (type === 'add') {
		...
		const res = await getPlatformShow({ id: row.id });
		if (res.code === 0) {
...
const changeStatus = async () => {
	...
	if (status.value) {
		// 启用操作
		try {
			const res = await postEnableWareHouse({ ids: [currentRow.value?.id] });
			if (res.code === 0) {
				// 清空选中
		...
		try {
			const res = await postDisableWareHouse({ ids: [currentRow.value?.id] });
			if (res.code === 0) {
				// 清空选中
				tableRef.value.clearSelection();
				getDataList();
...
const batchOperation = async (type: string) => {
	try {
		...
		if (type === 'enable') {
			...
			try {
				const res = await postEnableWareHouse({
			...
			try {
				const res = await postDisableWareHouse({
					ids: selectedRows.map((item: any) => item.id),
				});
				if (res.code === 0) {
					// 清空选中
