				...
				<div class="mx-auto mt-4">
					<div class="px-4">
						<div class="grid sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-3">
							<div
								class="p-6 mb-6 bg-gray-100 rounded-lg dark:bg-gray-800"
								v-for="tenant in state.dataList"
								:key="tenant.id"
							>
								<div class="flex items-center justify-between">
									...
									<div class="flex items-end">
										...
										<span class="mr-1 dark:text-gray-300">{{
											tenant.code
										}}</span>
									</div>
								</div>
								<p
									class="my-3 text-sm font-normal text-gray-500 dark:text-gray-400"
								>
									状态：
									{{
										status_type.find(
											(item: { value: string }) => item.value === tenant.status
										)?.label
									}}
								</p>
								<p
									class="my-3 text-sm font-normal text-gray-500 dark:text-gray-400"
								>
									有效期： {{ parseDate(tenant.startTime) }} -
									{{ parseDate(tenant.endTime) }}
								</p>
								<div
									class="flex items-center justify-between mt-6 text-sm font-semibold text-gray-900 dark:text-gray-100"
								>
									<div class="flex">
										<svg
											t="1710908265535"
...
//  table hook
const { getDataList, currentChangeHandle, sizeChangeHandle, downBlobFile } =
	useTable(state, null);
