		...
		>
			...
			>
				<el-input
					...
					:placeholder="$t('sysmenu.inputComponentTip')"
				/>
			</el-form-item>
		</el-form>
		<template #footer>
		...
		},
		{
			validator: (rule: any, value: any, callback: any) => {
				validateExist(
					...
					value,
					callback,
					state.ruleForm.menuId !== '' || state.ruleForm.menuType === '1'
				);
			},
			trigger: 'blur',
		},
		...
		},
		{
			validator: (rule: any, value: any, callback: any) => {
				if (
					...
					validateNull(state.ruleForm.component)
				) {
...
const onSubmit = async () => {
	...
	try {
		loading.value = true;
		const res = state.ruleForm.menuId
