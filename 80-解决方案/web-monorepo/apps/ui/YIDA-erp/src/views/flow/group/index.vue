		...
		<div class="layout-padding-auto layout-padding-view">
			...
			<el-tabs class="mt-4 ml-8" @tab-click="handleTabClick">
				<el-tab-pane
					...
					v-for="item in successGroupList"
					:key="item.id"
				>
					<template #label>
					...
					>
						...
						<el-table-column
							prop="createTime"
							:label="$t('flow.createTime')"
							show-overflow-tooltip
						></el-table-column>
						<el-table-column
							prop="rangeShow"
							:label="$t('flow.scopeOfUse')"
							show-overflow-tooltip
						>
							<template #default="{ row }">
								{{
									row.rangeShow && row.rangeShow.length > 0
										? row.rangeShow
										: $t('flow.allUser')
								}}
							</template>
...
const { getDataList, sizeChangeHandle, currentChangeHandle, tableStyle } =
	useTable(state);
