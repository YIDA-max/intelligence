 ...
 * @LastEditTime: 2025-10-11 17:39:54
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderConfirmRule\components\rule-model\model-info\base-info\orderTotalQuantityFilter\index.vue
 * @Description: 
 * 
...
const validateForm = async () => {
	...
	if (Number(formData.value.minPrice) > Number(formData.value.maxPrice) && formData.value.maxPrice && formData.value.minPrice) {
		...
		const getFormData = {
			totalNumber: `totalNumber >= ${formData.value.minPrice} && totalNumber <= ${formData.value.maxPrice}`,
		};
		return getFormData;
	} else if (formData.value.minPrice && !formData.value.maxPrice) {
		const getFormData = {
			totalNumber: `totalNumber >= ${formData.value.minPrice}`,
		};
		return getFormData;
	} else if (!formData.value.minPrice && formData.value.maxPrice) {
		const getFormData = {
			totalNumber: `totalNumber <= ${formData.value.maxPrice}`,
		};
		return getFormData;
	}
};
...
const setFormData = (data: Record<string, any>) => {
	if (!data || !data.totalNumber) {
		return '错误：无效的数据格式，缺少 totalNumber 字段';
	}

	const priceStr = data.totalNumber;

	// 使用正则表达式解析范围
	const regex = /totalNumber\s*([><]=?)\s*([\d.]+)(?:\s*&&\s*totalNumber\s*([><]=?)\s*([\d.]+))?/;
				...
				<el-col :span="24">
					<el-form-item :label="t('订单总数量')">
						...
						<el-row style="width: 100%">
							<el-col :span="22">
								<el-input type="number" v-model="formData.minPrice" style="width: 220px" placeholder="(含)最小值"> </el-input> ~
								<el-input type="number" v-model="formData.maxPrice" style="width: 220px" placeholder="(含)最大值"> </el-input>
