<template>
	<div class="layout-padding">
		<div class="layout-padding-auto layout-padding-view">
			<MergeRuleModel />
			<!-- search选择参数选择行 -->
			<el-row class="" v-show="showSearch">
				<el-form
					:inline="true"
						...
						<el-switch
							:disabled="
								row.status === 1 || row.status === '1'
									? !auth('orderMergeRule-disable')
									: !auth('orderMergeRule-enable')
							"
							v-model="row.status"
							:active-value="1"
							:inactive-value="0"
							...
							<el-dropdown @command="(type:any) => openDialog(type, row)">
								...
								<template #dropdown>
									<el-dropdown-menu>
										<el-dropdown-item
											command="log"
											v-if="auth('orderMergeRule-info')"
											>日志</el-dropdown-item
										>
										<el-dropdown-item
											command="copy"
											v-if="auth('orderMergeRule-copy')"
											>复制</el-dropdown-item
										>
									</el-dropdown-menu>
								</template>
...
import { getRuleConditionMergeDetail } from '/@/api/fbm/orderRule/orderMergeRule/index';
...
const MergeRuleModel = defineAsyncComponent(
	() => import('./components/merge-rule-model/index.vue')
);
...
const openDialog = async (type: string, row?: any) => {
	try {
		if (type === 'log') {
			...
			// 编辑规则
			const res = await getRuleConditionMergeDetail({
				id: row.id,
				type: 'merge',
			});
			if (res.code === 0) {
			...
			ElMessageBox.confirm(
				...
				.then(async () => {
					let res;
					if (row.status === 1) {
						res = await postRuleInfoOpen({ ids: [row.id], type: 'merge' });
					} else {
						res = await postRuleInfoClose({ ids: [row.id], type: 'merge' });
					}
					if (res.code === 0) {
			...
			ElMessageBox.confirm(
				t('是否确认禁用选中的规则？关闭合单规则，将移出等待合单订单，请确认'),
				{
					type: 'warning',
				}
			)
