			...
			<el-row>
				...
				>
					<template #left-tool>
						...
						>
						<el-button
							v-auth="'orderConfirmRule-enable'"
							@click="
								() => {
						...
						>
						<el-button
							v-auth="'orderConfirmRule-disable'"
							@click="
								() => {
						...
						<el-switch
							:disabled="
								row.status === 1 || row.status === '1'
									? !auth('orderConfirmRule-disable')
									: !auth('orderConfirmRule-enable')
							"
							v-model="row.status"
							:active-value="1"
							:inactive-value="0"
					...
					<template #default="{ row }">
						<div class="flex items-center">
							<el-link
								...
								v-if="auth('orderConfirmRule-edit')"
							>
								{{ $t('编辑') }}
							</el-link>
							<el-dropdown @command="(type:any) => openDialog(type, row)">
								...
								<template #dropdown>
									<el-dropdown-menu>
										<el-dropdown-item
											command="log"
											v-if="auth('orderConfirmRule-info')"
											>日志</el-dropdown-item
										>
										<el-dropdown-item
											command="copy"
											v-if="auth('orderConfirmRule-copy')"
											>复制</el-dropdown-item
										>
									</el-dropdown-menu>
								</template>
		...
		if (type === 'log') {
			...
			ElMessageBox.confirm(
				...
				.then(async () => {
					let res;
					if (row.status === 1) {
						res = await postRuleInfoOpen({ ids: [row.id], type: 'accept' });
					} else {
						res = await postRuleInfoClose({ ids: [row.id], type: 'accept' });
					}
					if (res.code === 0) {
