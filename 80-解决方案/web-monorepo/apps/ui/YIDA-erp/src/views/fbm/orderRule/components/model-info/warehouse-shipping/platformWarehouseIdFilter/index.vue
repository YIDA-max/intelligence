 ...
 * @LastEditTime: 2026-02-27 10:50:03
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\components\model-info\warehouse-shipping\platformWarehouseIdFilter\index.vue
 * @Description: 
 * 
...
const rules = ref({
	platformWareId: [
		{ required: true, message: '请选择平台仓库ID', trigger: 'blur' },
	],
});
const init = () => {
	// 重置数据
...
const validateForm = async () => {
	...
	if (formData.value.isBelong === '1') {
		const getFormData = {
			platformWareId: `platformWareId in [${formData.value.platformWareId.map(
				(item: string) => `'${item}'` // 去除前后空格 加上单引号
			)}]`,
		};
		return getFormData;
...
const setFormData = (data: Record<string, any>) => {
	...
	const excludeMatch = data.platformWareId.match(
		/!\(platformWareId in \[([^\]]+)\]\)/
	);
	if (excludeMatch) {
		formData.value.isBelong = '2';
		formData.value.platformWareId = excludeMatch[1]
			.split(',')
	...
	if (match) {
		...
		formData.value.platformWareId = match[1]
			.split(',')
			.map((item: string) => item.trim().replace(/'/g, '')); // 去除空格和引号
		return '成功设置数据';
	}
	...
	<div class="box">
		<el-form :model="formData" ref="formRef" label-width="150px" :rules="rules">
			<el-row class="base-info" :gutter="20">
				<el-col :span="24">
					<el-form-item :label="t('平台仓库ID')" prop="platformWareId">
						<el-row style="width: 100%">
							<el-col :span="24">
								支持批量复制不同分项按逗号分隔,按回车(Enter)键或点击鼠标完成输入
								(已填写{{ formData.platformWareId.length }} 项)
							</el-col>
						</el-row>
						<el-row style="width: 100%">
							...
							<el-col :span="20">
								<el-input-tag
									v-model="formData.platformWareId"
									trigger="Enter"
									@blur="handleBlur"
								/>
							</el-col>
