...
// 表单校验
const rules = ref({
	platformChannel: [{ required: true, message: '请选择订单平台', trigger: 'blur' }],
});
const init = () => {
	// 重置数据
	formData.value = {
...
const validateForm = async () => {
	...
	return isInToValidateForm(formData.value.isBelong, 'platformChannel', formData.value.platformChannel);
};
// 设置回显值
const setFormData = (data: Record<string, any>) => {
	// 设置数据
	return isInToSetFormData(data, 'platformChannel', formData);
};
// 完善的数据设置函数
	...
	<div class="box">
		<el-form :model="formData" ref="formRef" label-width="150px" :rules="rules">
			<el-row class="base-info" :gutter="20">
				<el-col :span="24">
					<el-form-item :label="t('订单平台')" prop="platformChannel">
						<el-row style="width: 100%">
							...
							<el-col :span="20">
								<el-select
									collapse-tags
									:max-collapse-tags="5"
									v-model="formData.platformChannel"
									clearable
									filterable
									multiple
									placeholder="平台渠道"
									style="width: 100%"
								>
									<el-option v-for="item in dictMap.platformChannel" :key="item.id" :label="item.label" :value="item.value" />
								</el-select>
							</el-col>
						</el-row>
					</el-form-item>
