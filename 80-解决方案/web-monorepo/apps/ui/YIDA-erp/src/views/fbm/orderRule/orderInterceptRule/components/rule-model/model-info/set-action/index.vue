...
const validateForm = async () => {
	...
	if (formData.value.autoFlag === 1 && !formData.value.cutOffReason) {
		return Promise.reject(t('请选择截单原因'));
	}
	const getFormData = {
		autoFlag: formData.value.autoFlag,
		cutOffReason: formData.value.cutOffReason,
...
const setFormData = (data: Record<string, any>) => {
	formData.value = {
		...
		tag: data.tagInfoVOS || [],
		cutOffReason: data.cutOffReason || '',
	};
}; // 暴露给父组件的方法
defineExpose({
	validateForm,
...
<template>
	<div class="box">
		<el-form :model="formData" ref="formRef" label-width="150px" :rules="rules">
						...
						<el-row style="width: 100%">
							<el-col :span="24">
								<el-radio-group v-model="formData.autoFlag">
									<el-radio :value="1" size="small">自动截单</el-radio>
								</el-radio-group>
							</el-col>
						</el-row>
					</el-form-item>
			...
			<el-row class="base-info mb-2" :gutter="20">
				<el-col :span="24">
					<el-form-item label="截单原因" prop="cutOffReason">
						<template #label>
							<span
								:style="{
									color: '#f78c8c',
						...
						<el-row style="width: 100%">
							<div class="flex items-center">
								<el-select
									...
									clearable
									filterable
									v-model="formData.cutOffReason"
									placeholder="截单原因"
									style="width: 500px"
								>
									<el-option
										v-for="item in [
											{ id: 1, label: '取消订单', value: '取消订单' },
											{ id: 2, label: '换地址', value: '换地址' },
											{ id: 3, label: '换货', value: '换货' },
											{ id: 4, label: '需要拆单', value: '需要拆单' },
											{ id: 5, label: 'SKU未找到', value: 'SKU未找到' },
