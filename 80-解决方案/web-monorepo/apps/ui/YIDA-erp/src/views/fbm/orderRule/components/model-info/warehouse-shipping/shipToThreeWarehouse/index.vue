...
// 表单校验
const rules = ref({
	warehouseCode: [{ required: true, message: '请选择三方仓仓库', trigger: 'blur' }],
});
const init = () => {
	// 重置数据
	formData.value = {
...
const validateForm = async () => {
	...
	return isInToValidateForm(formData.value.isBelong, 'warehouseCode', formData.value.warehouseCode);
};
// 设置回显值
const setFormData = (data: Record<string, any>) => {
	// 设置数据
	return isInToSetFormData(data, 'warehouseCode', formData);
};
// 完善的数据设置函数
	...
	<div class="box">
		<el-form :model="formData" ref="formRef" label-width="150px" :rules="rules">
			<el-row class="base-info" :gutter="20">
				<el-col :span="24">
					<el-form-item :label="t('三方仓仓库')" prop="warehouseCode">
