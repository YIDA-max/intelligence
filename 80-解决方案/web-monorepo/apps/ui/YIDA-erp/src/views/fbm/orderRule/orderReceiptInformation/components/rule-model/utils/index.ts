 ...
 * @LastEditTime: 2025-11-07 10:13:38
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderReceiptInformation\components\rule-model\utils\index.ts
 * @Description:
 *
...
export const initFn = async (
	...
	refs: {
		...
		shipToDoorCodeFilterRef: any; // 收件人门编号ref
	...
	const init = () => {
		...
		// 设置 收件人门编号ref
		refs.shipToDoorCodeFilterRef.value.init();
		// 设置 仓库物流ref
	...
	if (props.visibleType === 'add') {
		...
		refs.orderPlatformFilterRef.value.setFormData({
			platformChannel:
				props.currentRow.ruleConditionRecipient?.platformChannel || '',
		});
		// 设置 店铺站点ref
		refs.orderCountryRegionFilterRef.value.setFormData({
			site: props.currentRow.ruleConditionRecipient?.site || '',
		});
		// 设置 订单店铺ref
		refs.orderStoreFilterRef.value.setFormData({
			store: props.currentRow.ruleConditionRecipient?.store || '',
		});
		// 设置 订单访问方式ref
		refs.orderAccessMethodFilterRef.value.setFormData({
			orderFrom: props.currentRow.ruleConditionRecipient?.orderFrom || '',
		});
		// 设置 订单售价(不含税)ref
		refs.orderPriceExTaxFilterRef.value.setFormData({
			sellingPrice: props.currentRow.ruleConditionRecipient?.sellingPrice || '',
		});
		// 设置 订单总数量ref
		refs.orderTotalQuantityFilterRef.value.setFormData({
			totalNumber: props.currentRow.ruleConditionRecipient?.totalNumber || '',
		});
		// 设置 订单总重量ref
		refs.orderTotalWeightFilterRef.value.setFormData({
			weight: props.currentRow.ruleConditionRecipient?.weight || '',
		});
		// 设置 是否有买家备注ref
		refs.hasBuyerNoteFilterRef.value.setFormData({
			buyerNote: props.currentRow.ruleConditionRecipient?.buyerNote || '',
		});
		// 设置 是否定制订单ref
		refs.isCustomizedOrderFilterRef.value.setFormData({
			madeToOrder: props.currentRow.ruleConditionRecipient?.madeToOrder || '',
		});
		// 设置 订单体积重量ref
		refs.orderVolumetricWeightFilterRef.value.setFormData({
			volumetricWeight:
				props.currentRow.ruleConditionRecipient?.volumetricWeight || '',
			volumetricWeightParam:
				props.currentRow.ruleConditionRecipient?.volumetricWeightParam || '',
		});
		// 设置 商品尺寸ref
		refs.productSizeFilterRef.value.setFormData({
			productSize: props.currentRow.ruleConditionRecipient?.productSize || '',
			productSizeHtml:
				props.currentRow.ruleConditionRecipient?.productSizeHtml || '',
		});
		// 设置 收件人国家/地区ref
		refs.shipToCountryRegionFilterRef.value.setFormData({
			recipientCountry:
				props.currentRow.ruleConditionRecipient?.recipientCountry || '',
		});
		// 设置 收件人州/省ref
		refs.shipToStateProvinceFilterRef.value.setFormData({
			recipientState:
				props.currentRow.ruleConditionRecipient?.recipientState || '',
		});
		// 设置 收件人城市ref
		refs.shipToCityFilterRef.value.setFormData({
			recipientCity:
				props.currentRow.ruleConditionRecipient?.recipientCity || '',
		});
		// 设置 收件人邮编ref
		refs.shipToPostalCodeFilterRef.value.setFormData({
			recipientPostcode:
				props.currentRow.ruleConditionRecipient?.recipientPostcode || '',
		});
		// 设置 收件人门编号ref
		refs.shipToDoorCodeFilterRef.value.setFormData({
			recipientDoorCode:
				props.currentRow.ruleConditionRecipient?.recipientDoorCode || '',
		});
		// 设置 仓库物流ref
		...
		refs.shippingServiceLevelFilterRef.value.setFormData({
			shipmentPriority:
				props.currentRow.ruleConditionRecipient?.shipmentPriority || '',
		});
		// 设置 平台仓库ref
		refs.platformWarehouseIdFilterRef.value.setFormData({
			platformWareId:
				props.currentRow.ruleConditionRecipient?.platformWareId || '',
		});
		// 设置 电话号/手机号ref
		refs.phoneMobileFilterRef.value.setFormData({
			phone: props.currentRow.ruleConditionRecipient?.phone || '',
		});
		// 设置 订单MSKUref
		refs.orderMSKUFilterRef.value.setFormData({
			mskuCode: props.currentRow.ruleConditionRecipient?.mskuCode || '',
		});
		// 设置 订单商品ref
		refs.orderProductFilterRef.value.setFormData({
			skuCode: props.currentRow.ruleConditionRecipient?.skuCode || '',
		});
		// 设置 产品品类ref
		refs.productCategoryFilterRef.value.setFormData({
			category: props.currentRow.ruleConditionRecipient?.category || '',
		});
		// 设置 产品品牌ref
		refs.productBrandFilterRef.value.setFormData({
			brand: props.currentRow.ruleConditionRecipient?.brand || '',
		});
		// 设置 特殊属性ref
		refs.specialAttributesFilterRef.value.setFormData({
			specialAttribute:
				props.currentRow.ruleConditionRecipient?.specialAttribute || '',
		});
		// 设置 底部操作ref
		refs.setAction.value.setFormData({
			...props.currentRow.ruleActionRecipient, // 发货确认方式
		});
...
const getCheckList = (props: { currentRow: any }) => {
	...
	// 订单平台
	if (props.currentRow?.ruleConditionRecipient?.platformChannel) {
		checkList.push('platformChannel');
	}
	// 店铺站点
	if (props.currentRow?.ruleConditionRecipient?.site) {
		checkList.push('site');
	}
	// 订单店铺
	if (props.currentRow?.ruleConditionRecipient?.store) {
		checkList.push('store');
	}
	// 订单访问方式
	if (props.currentRow?.ruleConditionRecipient?.orderFrom) {
		checkList.push('orderFrom');
	}
	// 订单售价(不含税)
	if (props.currentRow?.ruleConditionRecipient?.sellingPrice) {
		checkList.push('sellingPrice');
	}
	// 订单总数量
	if (props.currentRow?.ruleConditionRecipient?.totalNumber) {
		checkList.push('totalNumber');
	}
	// 订单总重量
	if (props.currentRow?.ruleConditionRecipient?.weight) {
		checkList.push('weight');
	}
	// 是否有买家备注
	if (props.currentRow?.ruleConditionRecipient?.buyerNote) {
		checkList.push('buyerNote');
	}
	// 是否定制订单
	if (props.currentRow?.ruleConditionRecipient?.madeToOrder) {
	...
	// 商品尺寸
	if (
		props.currentRow?.ruleConditionRecipient?.productSize &&
		props.currentRow?.ruleConditionRecipient?.productSizeHtml
	) {
		checkList.push('productSize');
	}
	// 收件人国家/地区
	if (props.currentRow?.ruleConditionRecipient?.recipientCountry) {
		checkList.push('recipientCountry');
	}
	// 收件人州/省
	if (props.currentRow?.ruleConditionRecipient?.recipientState) {
		checkList.push('recipientState');
	}
	// 收件人城市
	if (props.currentRow?.ruleConditionRecipient?.recipientCity) {
	...
	// 收件人门编号
	if (props.currentRow?.ruleConditionRecipient?.recipientDoorCode) {
		checkList.push('recipientDoorCode');
	}
	// 仓库物流
	if (
		props.currentRow?.ruleConditionRecipient?.warehouseCode ||
	...
	// 发运级别
	if (props.currentRow?.ruleConditionRecipient?.shipmentPriority) {
		checkList.push('shipmentPriority');
	}
	// 平台仓库ID
	if (props.currentRow?.ruleConditionRecipient?.platformWareId) {
	...
	// 电话号/手机号ref
	if (props.currentRow?.ruleConditionRecipient?.phone) {
		checkList.push('phone');
	}
	// 订单MSKUref
	if (props.currentRow?.ruleConditionRecipient?.mskuCode) {
		checkList.push('mskuCode');
	}
	// 订单商品ref
	if (props.currentRow?.ruleConditionRecipient?.skuCode) {
		checkList.push('skuCode');
	}
	// 产品品类ref
	if (props.currentRow?.ruleConditionRecipient?.category) {
		checkList.push('category');
	}
	// 产品品牌ref
	if (props.currentRow?.ruleConditionRecipient?.brand) {
		checkList.push('brand');
	}
	// 特殊属性ref
	if (props.currentRow?.ruleConditionRecipient?.specialAttribute) {
