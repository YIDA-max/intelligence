 ...
 * @LastEditTime: 2026-05-28 15:07:50
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderReceiptInformation\components\rule-model\index.vue
 * @Description: 
 * 
...
import {
	postRuleConditionRecipientUpdate,
	postRuleConditionRecipientSave,
} from '/@/api/fbm/orderRule/orderReceiptInformation/index';
...
// 收货门编号
const ShipToDoorCodeFilter = defineAsyncComponent(
	() =>
		import(
			'/@/views/fbm/orderRule/components/model-info/warehouse-shipping/shipToDoorCodeFilter/index.vue'
		)
);
// 仓库物流
...
// 收货门编号ref
const shipToDoorCodeFilterRef = ref<any>(null);
// 仓库物流ref
...
const optionsGroups = ref([
	...
	},
	{
		...
		options: [
			...
			{ label: '收货门牌号', value: 'recipientDoorCode' },
...
const init = () => {
	...
	initFn(props, {
		...
		shipToDoorCodeFilterRef, // 收货门编号ref
...
const onSubmit = async () => {
	try {
		...
		//   获取收货门编号数据
		const shipToDoorCodeFilterData = currentCheckList.value.includes(
			'recipientDoorCode'
		)
			? (await shipToDoorCodeFilterRef.value?.validateForm()) || {}
			: {
					recipientDoorCode: '', // 收货门编号
			  };
		//   获取仓库物流数据
		...
		const params = {
			...
			ruleConditionRecipient: {
				...
				...shipToDoorCodeFilterData, // 收货门编号数据
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionRecipient.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionRecipient.ruleId
						: undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionRecipient: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionRecipient.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionRecipient.ruleId
						: undefined, // 规则id
			},
			type: 'recipient', // 确定是哪个规则单
		...
		const hasAnyCondition = Object.entries(params.ruleConditionRecipient).some(
			([key, val]) =>
	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" :model-title="t('收件信息')" />
				</el-col>
			</el-row>
				...
				<el-col :span="19" class="rule-info">
					...
					<el-row
						...
						v-show="
							...
							currentCheckList?.includes('recipientDoorCode') ||
					...
					<el-row v-show="currentCheckList?.includes('recipientDoorCode')">
						<el-col :span="24">
							<!-- 收货门编号 -->
							<ShipToDoorCodeFilter ref="shipToDoorCodeFilterRef" />
						</el-col>
					</el-row>
					<el-row
