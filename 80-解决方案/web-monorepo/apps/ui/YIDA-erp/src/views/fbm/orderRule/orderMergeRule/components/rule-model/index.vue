 ...
 * @LastEditTime: 2026-05-28 15:07:43
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderMergeRule\components\rule-model\index.vue
 * @Description: 
 * 
...
import {
	postRuleConditionMergeSave,
	postRuleConditionMergeUpdate,
} from '/@/api/fbm/orderRule/orderMergeRule/index';
...
const onSubmit = async () => {
	try {
			...
			ruleConditionMerge: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionMerge.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionMerge.ruleId
						: undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionMerge: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionMerge.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionMerge.ruleId
						: undefined, // 规则id
				type: 1,
			},
			type: 'merge', // 确定是哪个规则单
		...
		const hasAnyCondition = Object.entries(params.ruleConditionMerge).some(
			([key, val]) =>
	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" :model-title="t('合单规则')" />
				</el-col>
			</el-row>
