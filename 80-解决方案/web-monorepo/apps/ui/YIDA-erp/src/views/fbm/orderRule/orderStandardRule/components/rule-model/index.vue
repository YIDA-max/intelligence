 ...
 * @LastEditTime: 2026-05-28 15:08:30
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\orderStandardRule\components\rule-model\index.vue
 * @Description: 
 * 
...
import {
	postRuleConditionShippedUpdate,
	postRuleConditionShippedSave,
} from '/@/api/fbm/orderRule/orderStandardRule';
...
const onSubmit = async () => {
	try {
			...
			ruleConditionShipped: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionShipped.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleConditionShipped.ruleId
						: undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionShipped: {
				...
				id:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionShipped.id
						: undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit'
						? props.currentRow?.ruleActionShipped.ruleId
						: undefined, // 规则id
			},
			type: 'shipped', // 确定是哪个规则单
		...
		const hasAnyCondition = Object.entries(params.ruleConditionShipped).some(
			([key, val]) =>
	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" :model-title="t('标发规则')" />
				</el-col>
			</el-row>
