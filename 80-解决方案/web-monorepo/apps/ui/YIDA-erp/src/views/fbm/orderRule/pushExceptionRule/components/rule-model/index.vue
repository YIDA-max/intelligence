 ...
 * @LastEditTime: 2026-05-28 15:10:25
 * @FilePath: \qianyi-ui\src\views\fbm\orderRule\pushExceptionRule\components\rule-model\index.vue
 * @Description: 
 * 
...
import {
	postRuleConditionReviewSave,
	postRuleConditionReviewUpdate,
} from '/@/api/fbm/orderRule/pushExceptionRule/index';
...
// 比价结果
const ShipToTotalFee = defineAsyncComponent(
	() =>
		import(
			'/@/views/fbm/orderRule/components/model-info/warehouse-shipping/shipToTotalFee/index.vue'
		)
);
// 仓库物流
...
// 推单异常原因关键词
const ExceptionKeywordsFilter = defineAsyncComponent(
	() =>
		import(
			'/@/views/fbm/orderRule/components/model-info/base-info/pushExceptionReason/index.vue'
		)
);
...
// 推单异常原因关键词ref
const exceptionKeywordsFilterRef = ref<any>(null);
...
// 比价结果ref
const shipToTotalFeeRef = ref<any>(null);
// 仓库物流ref
...
const optionsGroups = ref([
	{
		...
		options: [
			...
			{ label: '推单异常原因关键词', value: 'exceptionKeywords' },
		],
	},
	{
		title: '仓库物流',
...
const init = () => {
	...
	initFn(props, {
		...
		exceptionKeywordsFilterRef, // 推单异常原因关键词ref
...
const onSubmit = async () => {
	try {
		...
		//   获取推单异常原因关键词数据
		const exceptionKeywordsFilterData = currentCheckList.value.includes(
			'exceptionKeywords'
		)
			? (await exceptionKeywordsFilterRef.value?.validateForm()) || {}
			: {
					exceptionKeywords: '', // 推单异常原因关键词
			  };
		//   获取收货国家/地区数据
		...
		//   获取比价结果数据
		const shipToTotalFeeData = currentCheckList.value.includes('totalFee')
			? (await shipToTotalFeeRef.value?.validateForm()) || {}
			: {
					calFee: '', // 比价结果
			  };
		//   获取仓库物流数据
		...
		const params = {
			...
			ruleConditionPushWmsMatch: {
				...
				...exceptionKeywordsFilterData, // 推单异常原因关键词数据
				...
				id: props.visibleType === 'edit' ? props.currentRow?.id : undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit' ? props.currentRow?.ruleId : undefined, // 规则id
			},
			// 规则执行对象 (设定动作)
			ruleActionPushWmsMatch: {
				...actionData, // 设定动作数据
				id: props.visibleType === 'edit' ? props.currentRow?.id : undefined, // 规则条件id
				ruleId:
					props.visibleType === 'edit' ? props.currentRow?.ruleId : undefined, // 规则id
		...
		const hasAnyCondition = Object.entries(
			params.ruleConditionPushWmsMatch
		).some(
			([key, val]) =>
		...
		// 推单异常原因关键词必须勾选且必填
		if (!currentCheckList.value.includes('exceptionKeywords')) {
			useMessage().error('请勾选并填写推单异常原因关键词');
			return;
		}
	...
	>
		<div class="rule-model">
			<el-row class="rule-title">
				<el-col :span="24">
					<ModelTitle ref="modelTitleRef" modelTitle="推单异常规则" />
				</el-col>
			</el-row>
				...
				<el-col :span="19" class="rule-info">
					<el-row
						...
						v-show="
							...
							currentCheckList?.includes('exceptionKeywords')
						"
					>
					...
					<el-row v-show="currentCheckList?.includes('exceptionKeywords')">
						<el-col :span="24">
							<!-- 推单异常原因关键词 -->
							<ExceptionKeywordsFilter ref="exceptionKeywordsFilterRef" />
						</el-col>
					</el-row>
					<el-row
						class="title"
					...
					<el-row v-show="currentCheckList?.includes('totalFee')">
						<el-col :span="24">
							<!-- 比价结果 -->
							<ShipToTotalFee ref="shipToTotalFeeRef" />
						</el-col>
					</el-row>
					<el-row
