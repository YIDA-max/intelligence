<!--
	...
	本文件：头程调拨「费用分摊」弹窗 — 录入多行费用 → postTransferFeeShare 试算 → 下部预览表；
		→ 确认后 postTransferFeeSave 持久化。
	必读父组件：materialFlow/firstMileLogistics/showInfo/index.vue — 使用 ref 调用 open(...) 与
		buildPreviewRowsAndColumns(...)，主表里「分摊合计」必须与预览摊平字段 + getPreviewSummaries 对齐，
		改 fee_${id}_* 或合计规则时务必同步修改该 index 内汇总逻辑。

	镜像文件（同 UI、不同接口模块）：materialFlow/secondMileLogistics/showInfo/components/FeeDialog.vue
		业务差异主要在 API 归属；表格/校验/摊平如无特别说明应对称维护。

	API 分层（全局搜导入名定位）：
		feeConfig、transferFeeList、getTransferFeeList、postTransferFeeShare、postTransferFeeSave
		均从 /@/api/materialFlow/firstMileLogistics 引入（勿误用 secondMile 模块）。
...
<!--
	...
	交互分区：
	1) 上部表格：录入/编辑多行费用（物流商、承担主体、费用项、分摊方式、入账方式、金额币种、备注）；
		锁定行 paymentRequestStatus=1（已请款）来自后端时不可编辑、不可删。
		paymentRequestStatus：1、已请款；2、未请款。
	2) 「预览分摊结果」：不写库，仅 postTransferFeeShare 试算并在下部表格展示（API 定义在 firstMileLogistics）。
	3) 下部表格：按发货明细分摊结果；左侧固定列为调拨明细，中间为每笔费用的动态多级列，
		汇总行算法见 getPreviewSummaries（须与 showInfo 主列表合计一致）。
	对外：defineExpose.open / buildPreviewRowsAndColumns，父组件 firstMileLogistics/showInfo/index.vue 会调用后者做汇总对齐。
-->
<template>
	...
	>
		...
		<div class="fee-dialog-content mb-2">
			...
			>
				<!-- 物流商列 -->
				<MrTableColumn label="物流商" fixed width="200">
					<template #header>
					...
					<template #default="{ row }">
						<el-select
							v-model="row.logisticsProviderId"
							placeholder="请选择"
							clearable
							filterable
		...
		<div class="mt-2">
			...
			<!--
				show-summary：底部合计行；
				fee_${feeId}_* 字段由 buildPreviewRowsAndColumns 写入行对象，与子列 prop 对齐。
			-->
			<MrTable
				:data="previewList"
				border
			...
			>
				...
				<MrTableColumn label="MSKU" prop="mskuCode" width="100" />
				<MrTableColumn label="数量" prop="deliveryQuantity" width="100" />
				<MrTableColumn label="商品尺寸(CM)" width="150" show-overflow-tooltip>
					<template #default="{ row }">
						{{ formatPreviewDimensions(row) }}
					</template>
				</MrTableColumn>
				<MrTableColumn label="商品重量(KG)" prop="weight" width="150" />
				<MrTableColumn label="总体积(m³)" prop="volumeSum" width="120" />
				<MrTableColumn label="总重量(KG)" prop="weightSum" width="110" />
				<!-- 按费用项拆分多级表头：每个 feeCol 对应一行录入里的一个费用 -->
				<el-table-column
					v-for="feeCol in previewFeeColumns"
					:key="feeCol.feeId"
					:label="feeCol.feeName"
					align="center"
				>
					<el-table-column
						label="分摊方式"
 ...
 *
 * 数据流简述：
 * - open() 拉取「已保存费用行」+「已有分摊预览」双接口（firstMileLogistics），填充 tableData / previewList。
 * - handlePreview() / handleConfirm 前半段：用当前 tableData 调用 postTransferFeeShare，再 buildPreviewRowsAndColumns 更新预览。
 * - handleConfirm：先校验可编辑行 → 再隐式预览一遍 → 组装 lastMileFeeDTOList（含每行 lastMileFeeShareItemList）→ postTransferFeeSave。
 *
 * 暴露给父组件：
 * - open(type, id, transferNo)
 * - buildPreviewRowsAndColumns(data) 摊平行与动态列元数据（勿改返回结构，showInfo/index 汇总依赖）
 */
...
// 头程：费用项配置、列表、试算、保存（同模块内聚合导出）
import { feeConfig } from '/@/api/materialFlow/firstMileLogistics';
import {
	getLastMileFeeList,
	getLastMileFeeShareList,
	postLastMileFeeShare,
	postLastMileFeeSave,
} from '/@/api/materialFlow/logisticsFee/lastMileFee';
// 表头批量选择、申报币种符号展示
...
interface TransferFeeShareVO {
	...
	skuCode?: string;
	skuName?: string;
	lastMileFeeShareItemList?: TransferFeeShareItemVO[];
	transferNo?: string;
	lastMileTotalFee?: number;
	volumeSum?: number;
	[property: string]: any;
}
...
/* -------------------------------------------------------------------------- */
/* 预览：分摊方式文案、lastMileFeeShareItemList → 行摊平 fee_${id}_*、表尾合计       */
/* -------------------------------------------------------------------------- */
...
/**
 * 核心：接口返回的嵌套 lastMileFeeShareItemList → 摊平到行上的 prop 字段 + 生成 v-for 用的列配置
...
const buildPreviewRowsAndColumns = (data: TransferFeeShareVO[]) => {
	 ...
	 */
	const getFeeMetaByIndex = (index: number) => {
		const feeRow = tableData.value[index];
		const fallbackId = index + 1;
		const rawId = Number(feeRow?.feeId);
		const feeId = Number.isFinite(rawId) && rawId > 0 ? rawId : fallbackId;
		const feeName =
			dictMap.value?.feeList?.find((item: any) => Number(item.id) === feeId)
				?.feeItem ||
			feeRow?.feeName ||
			`费用${index + 1}`;
		return { feeId, feeName };
	};
	...
	const rows = (data || []).map((row) => {
		...
		for (const [index, item] of (
			row.lastMileFeeShareItemList || []
		).entries()) {
			const { feeId } = getFeeMetaByIndex(index);
			shareItemMap[feeId] = item;
			dynamicCellValues[`fee_${feeId}_shareType`] = getShareTypeText(
				item.shareType,
				item.shareTypeDesc
			);
			dynamicCellValues[`fee_${feeId}_shareUnitPrice`] =
				item.shareUnitPrice ?? '--';
			dynamicCellValues[`fee_${feeId}_shareTotalPrice`] =
				item.shareTotalPrice ?? '--';
			dynamicCellValues[`fee_${feeId}_exchangeRate`] = getExchangeRate(item);
			dynamicCellValues[`fee_${feeId}_currency`] = item.currency ?? 'CNY';
			dynamicCellValues[`fee_${feeId}_accountsReceiptTypeDesc`] =
	...
	const columns = tableData.value.map((_, index) => {
		const { feeId, feeName } = getFeeMetaByIndex(index);
		return {
			feeId,
			feeName,
			shareTypeProp: `fee_${feeId}_shareType`,
			shareUnitPriceProp: `fee_${feeId}_shareUnitPrice`,
			shareTotalPriceProp: `fee_${feeId}_shareTotalPrice`,
			exchangeRateProp: `fee_${feeId}_exchangeRate`,
			currencyProp: `fee_${feeId}_currency`,
			accountsReceiptTypeDesc: `fee_${feeId}_accountsReceiptTypeDesc`,
		};
	});
	return { rows, columns };
};
...
const resolveAllocationColumnProp = (column: {
	...
	if (label === '费用分摊总价') return 'lastMileTotalFee';
	return '';
};
...
const getPreviewSummaries = ({
	...
	columns.forEach((column, index) => {
		...
		// 与单元格一致：{{ getCurrencySymbol(row.currency) }}{{ row.lastMileTotalFee }}
		if (prop === 'lastMileTotalFee') {
...
const open = async (
	...
	dialogTitle.value =
		type === 'estimate'
			...
			: '';
	transferNo.value = firstNo;
	// ① 尾程实际费用项列表查询：/last-mile/fee
	const resTransferFeeList = await getLastMileFeeList({
		orderNo: transferNo.value,
	...
	// ② 尾程实际费用分摊列表查询：/last-mile/fee/share
	const resTransferFeeShareList = await getLastMileFeeShareList({
		orderNo: transferNo.value,
		feeType: type === 'estimate' ? 1 : 2,
	});
	// 仅处理标准返回结构：{ code, data }
...
const handlePreview = async () => {
	...
	const res = await postLastMileFeeShare(
...
/* -------------------------------------------------------------------------- */
/* 行内编辑联动 + 表头 BatchPopover 批量写入（跳过锁定行）                        */
/* -------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
/* 保存：从预览表按 feeId 抽取 lastMileFeeShareItemDTOList、回填汇率              */
/* -------------------------------------------------------------------------- */

/**
 * 根据费用项 ID 组装保存所需的分摊明细列表
 * lastMileFeeShareItemDTOList 不能为空，因此这里从“预览结果”中按 feeId 提取对应明细
 */
const buildLastMileFeeShareItemDTOList = (feeId: number) => {
	const rows = previewList.value || [];

	return rows
		.map((row: any) => {
			const shareItem = row?.__shareItemMap?.[feeId];

			if (!shareItem) return null;

			return {
				// 按 lastMileFeeShareItemList VO 结构输出
				deliveryQuantity: Number(row?.deliveryQuantity ?? 0),
				exchangeRate: shareItem?.exchangeRate ?? null,
				mSkuCode: row?.mskuCode ?? null,
				perimeter: Number(row?.perimeter ?? 0),
...
const handleConfirm = async () => {
	...
	try {
		// 与后台约定：保存前需以最新 tableData 再跑一遍试算，确保 lastMileFeeShareItemDTOList 与 preview 对齐
		await handlePreview();
		// 费用分摊模式：lastMileFeeShareItemDTOList 依赖预览结果
		if (!previewList.value?.length) {
			msg.warning('请先预览分摊结果');
			return;
		}
		// tableData 是上面录入的数据，进行反查，然后拼装
		const lastMileFeeDTOList = editableRows.map((item) => {
			const feeId = Number(item.feeId);
			const lastMileFeeShareItemDTOList =
				buildLastMileFeeShareItemDTOList(feeId);
			...
			return {
				logisticsProviderId: Number(item.logisticsProviderId),
				legalPersonId: Number(item.legalPersonId),
				feeId,
				shareType: Number(item.shareType),
				accountsReceiptType: Number(item.accountsReceiptType),
				...
				exchangeRate,
				// 新增：保存接口要求该字段不能为空
				lastMileFeeShareItemDTOList,
			};
		});
		// 兜底校验：任一费用项没有匹配到分摊明细时，阻断提交
		if (
			lastMileFeeDTOList.some(
				(item) =>
					!item.lastMileFeeShareItemDTOList ||
					item.lastMileFeeShareItemDTOList.length === 0
			)
		) {
			msg.warning('分摊明细不能为空，请先预览并检查费用项');
			return;
		}
		const res = await postLastMileFeeSave({
			feeType: feeType.value === 'estimate' ? 1 : 2,
			orderNo: transferNo.value,
			lastMileFeeDTOList,
		});
