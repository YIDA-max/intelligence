...
/**
 * 解析命令行传入的环境参数
 * @returns {'dev' | 'prod' | string | null} 如果传了参数返回参数值，否则返回 null
 */
...
/**
 * 交互式 CLI 选择构建选项
 * @returns {Promise<{env: string, shouldZip: boolean, outDir: string}>}
 */
async function promptBuildOptions() {
	const cliEnv = getEnvironmentFromArgs();

	// 如果命令行已传参，跳过交互（向后兼容）
	if (cliEnv) {
		console.log(`📦 使用命令行参数: ${cliEnv}`);
		return {
			env: cliEnv,
			shouldZip: true,
			outDir: 'html',
	...
	const response = await prompts([
		...
		},
		{
			type: 'select',
			name: 'shouldZip',
			message: '是否打包为 ZIP 文件？',
			choices: [
				{ title: '是', value: true, description: '构建完成后自动压缩为 ZIP' },
				{ title: '否', value: false, description: '仅构建，不压缩' },
			],
			initial: 0,
		},
		{
	...
	return {
		env: response.env,
		shouldZip: response.shouldZip ?? true,
		outDir: response.outDir || 'html',
	};
}
...
async function main() {
	// 交互式选择构建选项
	const buildOptions = await promptBuildOptions();
	const { env: cliEnv, shouldZip, outDir } = buildOptions;
	...
	try {
		...
		if (shouldZip) {
			...
			await zipBuildOutput(projectRoot, outDir);
		} else {
			console.log(`\n✅ 构建完成，输出目录: ${outDir}`);
		}
	} catch (error) {
